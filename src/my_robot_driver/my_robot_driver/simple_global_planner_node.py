...
TEMP_OBSTACLE_LETHAL_CORE_RADIUS_M = 0.25  # For A* planning
TEMP_OBSTACLE_INFLATION_COST_AT_CORE_EDGE = 230  # For A* planning
...
class SimpleGlobalPlanner(Node):
    def __init__(self):
        ...
        self.declare_parameter("simplification_max_allowed_cost", 100)  # Increased
        self.declare_parameter("temp_avoidance_topic", "/temp_avoidance_points")
        self.declare_parameter(
            "temp_avoidance_radius_m", 0.50
        )  # A* outer inflation boundary for temp
        self.declare_parameter("temp_avoidance_point_lifetime_s", 7.0)
        self.declare_parameter("astar_turn_penalty_cost", 7.0)

        # New parameters for enhanced simplification
        self.declare_parameter(
            "simplification_temp_obstacle_clearance_radius_m", 0.25
        )  # For LOS check against temp obstacles
        self.declare_parameter(
            "simplification_min_angle_change_deg", 5.0
        )  # For angle-based simplification
        ...
        self.cost_penalty_multiplier = self.get_parameter(
            "cost_penalty_multiplier"
        ).value
        self.enable_path_simplification = self.get_parameter(
            "enable_path_simplification"
        ).value
        self.simplification_obstacle_check_expansion_cells = self.get_parameter(
            "simplification_obstacle_check_expansion_cells"
        ).value
        self.simplification_max_allowed_cost = self.get_parameter(
            "simplification_max_allowed_cost"
        ).value
        self.temp_avoidance_topic_name = self.get_parameter(
            "temp_avoidance_topic"
        ).value
        self.temp_avoidance_radius_m = self.get_parameter(
            "temp_avoidance_radius_m"
        ).value
        self.temp_avoidance_point_lifetime_s = self.get_parameter(
            "temp_avoidance_point_lifetime_s"
        ).value
        self.astar_turn_penalty_cost = self.get_parameter(
            "astar_turn_penalty_cost"
        ).value
        self.simplification_temp_obstacle_clearance_radius_m = self.get_parameter(
            "simplification_temp_obstacle_clearance_radius_m"
        ).value
        self.simplification_min_angle_change_deg = self.get_parameter(
            "simplification_min_angle_change_deg"
        ).value

        # Sanity checks for new simplification radius
        if (
            self.simplification_temp_obstacle_clearance_radius_m
            < TEMP_OBSTACLE_LETHAL_CORE_RADIUS_M
        ):
            self.get_logger().warn(
                f"simplification_temp_obstacle_clearance_radius_m ({self.simplification_temp_obstacle_clearance_radius_m}) "
                f"is < TEMP_OBSTACLE_LETHAL_CORE_RADIUS_M ({TEMP_OBSTACLE_LETHAL_CORE_RADIUS_M}). Adjusting clearance to core radius."
            )
            self.simplification_temp_obstacle_clearance_radius_m = (
                TEMP_OBSTACLE_LETHAL_CORE_RADIUS_M
            )
        if (
            self.simplification_temp_obstacle_clearance_radius_m
            > self.temp_avoidance_radius_m
        ):
            self.get_logger().warn(
                f"simplification_temp_obstacle_clearance_radius_m ({self.simplification_temp_obstacle_clearance_radius_m}) "
                f"is > temp_avoidance_radius_m ({self.temp_avoidance_radius_m}). Simplification might be too constrained. Adjusting to temp_avoidance_radius_m."
            )
            self.simplification_temp_obstacle_clearance_radius_m = (
                self.temp_avoidance_radius_m
            )
        ...
        self.temp_avoidance_sub = self.create_subscription(
            Float32MultiArray,
            self.temp_avoidance_topic_name,
            self._temp_avoidance_callback,
            10,
        )
    ...
    def _temp_avoidance_callback(self, msg: Float32MultiArray):  # Unchanged
        if len(msg.data) == 2:
            x, y = msg.data[0], msg.data[1]
            self.get_logger().info(
                f"Received temporary avoidance point: ({x:.2f}, {y:.2f}) in map frame."
            )
            self.temporary_avoidance_points = [
                p
                for p in self.temporary_avoidance_points
                if not (
                    math.isclose(p[0], x, abs_tol=0.1)
                    and math.isclose(p[1], y, abs_tol=0.1)
                )
            ]
            self.temporary_avoidance_points.append((x, y, self.get_clock().now()))
        else:
            self.get_logger().warn(
                f"Invalid temp_avoidance_point msg. Expected 2 floats, got {len(msg.data)}."
            )

    def _cleanup_old_avoidance_points(self):  # Unchanged
        ...
        self.temporary_avoidance_points = [
            p
            for p in self.temporary_avoidance_points
            if (now - p[2]) < lifetime_duration
        ]
        if initial_count > len(self.temporary_avoidance_points):
            self.get_logger().debug(
                f"Cleaned up {initial_count - len(self.temporary_avoidance_points)} old avoidance points."
            )

    def _apply_temporary_avoidances(
        self,
    ):  # Unchanged (uses TEMP_OBSTACLE_LETHAL_CORE_RADIUS_M and self.temp_avoidance_radius_m for A* planning)
            ...
            if mc:
                ...
                for ro in range(-outer_r_cells, outer_r_cells + 1):
                    for co in range(-outer_r_cells, outer_r_cells + 1):
                        ...
                        if dist_cells > outer_r_cells:
                            continue
                        if self.planning_costmap[r, c] >= LETHAL_OBSTACLE_COST_INTERNAL:
                            continue
                        tmp_pen = 0
                            ...
                            if infl_band_w < 1e-3:
                                ...
                                f = max(
                                    ...
                                    min(
                                        1.0,
                                        1.0
                                        - (dist_cells - lethal_core_r_cells)
                                        / infl_band_w,
                                    ),
                                )
                                infl_v = int(
                                    (
                                        TEMP_OBSTACLE_INFLATION_COST_AT_CORE_EDGE
                                        - self.cost_neutral
                                    )
                                    * f
                                    + self.cost_neutral
        ...
        if applied_cnt > 0:
            self.get_logger().info(
                f"Applied {applied_cnt} INFLATED temporary avoidance zones."
            )

    def _map_callback(self, msg: OccupancyGrid):  # Unchanged
        self.get_logger().info(
            f"Map: {msg.info.width}x{msg.info.height}, res: {msg.info.resolution}, frame: '{msg.header.frame_id}'"
        )
        if msg.header.frame_id != self.global_frame:
            self.get_logger().warn(f"Map frame mismatch.")
            return
        self.map_data, self.map_info = msg, msg.info
        self._create_and_inflate_costmap()

    def _create_and_inflate_costmap(self):  # Unchanged
        if (
            self.map_data is None
            or self.map_info is None
            or self.map_info.resolution == 0
        ):
            self.get_logger().warn("Map data NA.")
            return
        ...
        self.base_costmap[raw_map >= self.cost_lethal_threshold] = (
            LETHAL_OBSTACLE_COST_INTERNAL
        )
    ...
    def _inflate_obstacles_on_basemap(self):  # Unchanged
        if (
            self.base_costmap is None
            or self.map_info is None
            or self.map_info.resolution == 0
        ):
            return
        ...
        infl_r_cells = int(
            math.ceil(self.inflation_radius_m / self.map_info.resolution)
        )
        if infl_r_cells == 0:
            return
            ...
            for dr_ in [-1, 0, 1]:
                for dc_ in [-1, 0, 1]:
                    ...
                    if 0 <= nr < h and 0 <= nc < w and not visited[nr, nc]:
                        ...
                        cv = int(
                            (self.cost_inflated_max - self.cost_neutral) * f_
                            + self.cost_neutral
                        )
    ...
    def _world_to_map(self, wx, wy) -> tuple[int, int] | None:  # Unchanged
        ...
        if 0 <= mc < self.map_info.width and 0 <= mr < self.map_info.height:
            return mr, mc
        return None

    def _map_to_world(self, mr, mc) -> tuple[float, float] | None:  # Unchanged
        if self.map_info is None:
            return None
        ox, oy, res = (
            self.map_info.origin.position.x,
    ...
    def _get_robot_pose_in_global_frame(self) -> Pose | None:  # Unchanged
        try:
            now = rclpyTime()
    ...
    def _is_cell_traversable(
        self, r, c, exp, thresh, cmap
    ) -> bool:  # Unchanged (takes cmap)
        if cmap is None:
            return False
        for dro in range(-exp, exp + 1):
            for dco in range(-exp, exp + 1):
                nr, nc = r + dro, c + dco
                if not (0 <= nr < cmap.shape[0] and 0 <= nc < cmap.shape[1]):
                    return False
                if cmap[nr, nc] >= thresh:
                    return False
        return True

    # --- MODIFIED _is_cell_within_any_temp_avoidance_zone ---
    def _is_cell_within_any_temp_avoidance_zone(self, r: int, c: int) -> bool:
        if (
            not self.temporary_avoidance_points
            or self.map_info is None
            or self.map_info.resolution == 0
        ):
            return False
        ...
        if world_coords is None:
            return True  # Treat out-of-bounds as blocked for simplification safety

        world_x, world_y = world_coords

        # Use the NEW specific radius for simplification checks against temporary obstacles
        # This comes from the 'simplification_temp_obstacle_clearance_radius_m' parameter
        radius_for_check = self.simplification_temp_obstacle_clearance_radius_m

        # Apply expansion for safety if simplification_obstacle_check_expansion_cells > 0
        # This ensures the expanded footprint of the cell being checked is clear of the radius_for_check
        if (
            self.simplification_obstacle_check_expansion_cells > 0
            and self.map_info.resolution > 0
        ):
            radius_for_check += (
                self.simplification_obstacle_check_expansion_cells
                * self.map_info.resolution
                * 0.5
            )
        ...
        for avoid_x_center, avoid_y_center, _ in self.temporary_avoidance_points:
            ...
            if dist_sq <= final_check_radius_sq:
                return True
        return False

    # --- END MODIFIED ---

    def _bresenham_line_check(
        self, p1_rc, p2_rc
    ) -> bool:  # Unchanged (uses the modified _is_cell_within_any_temp_avoidance_zone)
        cmap_static = self.base_costmap
        if cmap_static is None:
            return False
        r1, c1 = p1_rc
        r2, c2 = p2_rc
        ...
        while s_taken < max_s:
            if not self._is_cell_traversable(cr, cc, s_exp, s_thresh, cmap_static):
                return False
            if self._is_cell_within_any_temp_avoidance_zone(cr, cc):
                return False  # Uses new radius internally
            if cr == r2 and cc == c2:
                return True
            e2 = 2 * err
            pr, pc = cr, cc
            ...
            if s_exp == 0 and cr != pr and cc != pc:
                if not self._is_cell_traversable(
                    pr, cc, 0, s_thresh, cmap_static
                ) or self._is_cell_within_any_temp_avoidance_zone(pr, cc):
                    return False
                if not self._is_cell_traversable(
                    cr, pc, 0, s_thresh, cmap_static
                ) or self._is_cell_within_any_temp_avoidance_zone(cr, pc):
                    return False
            s_taken += 1
        return False

    def _simplify_path_los(self, path_cells) -> list:  # Unchanged
        ...
        while curr_idx < len(path_cells) - 1:
            ...
            for next_cand_idx in range(curr_idx + 2, len(path_cells)):
                if self._bresenham_line_check(
                    path_cells[curr_idx], path_cells[next_cand_idx]
                ):
    ...
    # --- NEW ANGLE-BASED SIMPLIFICATION ---
    ...
    def _simplify_path_by_angle(
        self, path_cells: list[tuple[int, int]], min_angle_change_rad: float
    ) -> list[tuple[int, int]]:
        if (
            len(path_cells) <= 2 or min_angle_change_rad <= 0
        ):  # Added check for min_angle_change_rad
            return path_cells

        simplified_path = [path_cells[0]]  # Always keep the first point
        for i in range(
            1, len(path_cells) - 1
        ):  # Iterate up to the second to last point
            p_prev = path_cells[i - 1]
            p_curr = path_cells[i]
            p_next = path_cells[i + 1]

            # Vector from prev to curr (dx, dy) -> (dc, dr) for atan2(y,x) like behavior
            v1_dc, v1_dr = p_curr[1] - p_prev[1], p_curr[0] - p_prev[0]
            # Vector from curr to next
            v2_dc, v2_dr = p_next[1] - p_curr[1], p_next[0] - p_curr[0]

            # Avoid issues with zero-length vectors if points are identical
            if (v1_dr == 0 and v1_dc == 0) or (v2_dr == 0 and v2_dc == 0):
                # If p_curr is same as p_prev, or p_next is same as p_curr, effectively a zero angle change segment
                # but it might be a duplicate point. We'll keep p_curr for now if the previous point was different.
                # If simplified_path[-1] is already p_curr, this logic might be complex.
                # A simpler approach: if a segment is zero length, the point is redundant if angles are also same.
                # Let's assume distinct points from A* and LOS for now.
                # If v1 is zero length, it means p_curr = p_prev. We should have added p_prev. Don't add p_curr if angle check would remove it.
                # If v2 is zero length, it means p_next = p_curr. We'll add p_curr, and then p_next (which is same) will be the last point.
                if not (
                    v1_dr == 0 and v1_dc == 0
                ):  # Only proceed if first segment has length
                    simplified_path.append(
                        p_curr
                    )  # Keep if first segment exists, second is zero (end of a line)
                continue

            angle1 = math.atan2(v1_dr, v1_dc)  # atan2(y,x) -> atan2(row_diff, col_diff)
            angle2 = math.atan2(v2_dr, v2_dc)

            angle_diff = abs(self._normalize_angle_pi(angle2 - angle1))

            # If the angle change is significant, keep p_curr
            if angle_diff > min_angle_change_rad:
                if (
                    not simplified_path or simplified_path[-1] != p_curr
                ):  # Avoid duplicate points if any
                    simplified_path.append(p_curr)
            # else: point p_curr is removed as it's nearly collinear

        # Always keep the last point, ensure no duplicates if it was already added
        if not simplified_path or simplified_path[-1] != path_cells[-1]:
            if (
                len(path_cells) > 1
            ):  # only add last point if path has more than one point
                simplified_path.append(path_cells[-1])

        # Final check for consecutive duplicate points (can happen with edge cases)
        if len(simplified_path) > 1:
            final_cleaned_path = [simplified_path[0]]
            for i in range(1, len(simplified_path)):
                if simplified_path[i] != simplified_path[i - 1]:
                    final_cleaned_path.append(simplified_path[i])
            simplified_path = final_cleaned_path

        if len(simplified_path) < len(path_cells):  # Check original path_cells len
            self.get_logger().info(
                f"Angle Simp: Input {len(path_cells)} -> Output {len(simplified_path)} pts."
            )
    ...
    def _goal_callback(self, msg: PoseStamped):
        if self.base_costmap is None or self.map_info is None or self.map_data is None:
            ...
            return
        if msg.header.frame_id != self.global_frame:
            self.get_logger().warn(f"Goal frame mismatch.")
            return
        ...
        if self.planning_costmap is None:
            self.get_logger().error("No planning_costmap.")
            return
        ...
        if start_coords is None or goal_coords is None:
            self.get_logger().error("Start/Goal outside map.")
            return
        start_r, start_c = start_coords
        goal_r, goal_c = goal_coords
        self.get_logger().info(
            f"Planning from ({start_r},{start_c}) to ({goal_r},{goal_c})."
        )
        if (
            self.planning_costmap[start_r, start_c] >= LETHAL_OBSTACLE_COST_INTERNAL
            or self.planning_costmap[goal_r, goal_c] >= LETHAL_OBSTACLE_COST_INTERNAL
        ):
            self.get_logger().error(
                f"Start/Goal in lethal. S:{self.planning_costmap[start_r, start_c]}, G:{self.planning_costmap[goal_r, goal_c]}"
            )
            return
        ...
        if path_cells:
            ...
            if self.enable_path_simplification and len(path_cells) > 2:
                ...
                # Apply angle simplification if LOS simplification was effective enough
                if len(simplified_cells) > 2:
                    min_angle_rad = math.radians(
                        self.simplification_min_angle_change_deg
                    )
                    if (
                        min_angle_rad > 1e-3
                    ):  # Only apply if a meaningful threshold is set
                        simplified_cells = self._simplify_path_by_angle(
                            simplified_cells, min_angle_rad
                        )

            self.get_logger().info(
                f"Final simplified path: {len(simplified_cells)} points."
            )
        ...
        if final_path_to_publish:  # Check if list is not empty
        ...
        self.path_pub.publish(
            ros_path
        )  # Publish path (even if empty, Nav understands this as no path)
        self.publish_planning_costmap_for_debug()

    def _heuristic(self, p1, p2):
        return abs(p1[0] - p2[0]) + abs(p1[1] - p2[1])  # Unchanged

    def _find_path_astar(
        self, start_rc, goal_rc
    ):  # Unchanged (uses self.astar_turn_penalty_cost)
        if self.planning_costmap is None:
            return []
        open_set = []
        heapq.heappush(
            open_set, (self._heuristic(start_rc, goal_rc), 0, start_rc, None)
        )
        came_from = {}
        ...
        while open_set:
            proc_nodes += 1
            if proc_nodes > max_proc:
                self.get_logger().warn(f"A* max nodes ({proc_nodes})")
                return []
            f, g_tie, curr, parent = heapq.heappop(open_set)
            if g_tie > g_score[curr]:
                continue
            if curr == goal_rc:
            ...
            for dr_ in [-1, 0, 1]:
                for dc_ in [-1, 0, 1]:
                    ...
                    if ccv >= LETHAL_OBSTACLE_COST_INTERNAL:
                        continue
                    m_cost = math.sqrt(dr_**2 + dc_**2)
                    add_pen = (
                        (float(ccv) - self.cost_neutral) * self.cost_penalty_multiplier
                        if ccv > self.cost_neutral
                        else 0
                    )
                    turn_pen = 0
                    if parent is not None and self.astar_turn_penalty_cost > 1e-3:
                        ...
                        if not (
                            np.sign(vpcd_r) == np.sign(dr_)
                            and np.sign(vpcd_c) == np.sign(dc_)
                        ):
    ...
    def _reconstruct_path(self, came_from, current_rc):  # Unchanged
        path = [current_rc]
    ...
    def publish_planning_costmap_for_debug(
        self,
    ):  # Unchanged (minor self.debug_pub rename if issue)
        if (
            self.planning_costmap is None
            or self.map_info is None
        ...
        tim = (
            (self.planning_costmap > self.cost_neutral)
        ...
        viz[tcm] = 99
        if np.any(tim):
            c = self.planning_costmap[tim]
            mn, mx = float(np.min(c)), float(np.max(c))
            s = (
                np.full_like(c, 85, dtype=float)
                if mx - mn < 1e-3
                else 70.0 + ((c.astype(float) - mn) / (mx - mn)) * (98.0 - 70.0)
            )
            viz[tim] = np.clip(s, 70, 98).astype(np.int8)
        if np.any(sim):
            c = self.planning_costmap[sim]
            mn, mx = float(np.min(c)), float(np.max(c))
            s = (
                np.full_like(c, 35, dtype=float)
                if mx - mn < 1e-3
                else 1.0 + ((c.astype(float) - mn) / (mx - mn)) * (69.0 - 1.0)
            )

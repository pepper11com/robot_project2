...
from .drivetrain import Drivetrain          # Assuming drivetrain.py is in the same package directory
...
class MotorControllerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('Motor controller starting (4WD – stationary turning – ROBOT FLIPPED 180 DEG)…')

        # ---------- tunable parameters ------------------------------------
        self.declare_parameter('wheel_base',           0.13)
        self.declare_parameter('wheel_radius',         0.0325)
        self.declare_parameter('max_wheel_lin_speed',  0.026)
        self.declare_parameter('fixed_turn_pwm',       0.55)  # Increased from 0.48 to 0.55
        ...
        self.max_wheel_lin_speed  = max(0.001, self.get_parameter('max_wheel_lin_speed').value)
        self.fixed_turn_pwm       = clip(self.get_parameter('fixed_turn_pwm').value, 0.0, 1.0)
        self.fixed_forward_pwm    = clip(self.get_parameter('fixed_forward_pwm').value, 0.0, 1.0)
    ...
    def _on_cmd(self, msg: Twist):
        ...
        if turn_only and abs(w_cmd) > 0.01:
            ...
            turn_scale = min(abs(w_cmd) / 0.25, 1.0)  # Normalize to max expected angular vel
            actual_turn_pwm = max(self.fixed_turn_pwm * turn_scale, self.fixed_turn_pwm * 0.9)  # Min 90% of fixed (was 85%)
            ...
            forward_scale = min(abs(v_input) / 0.012, 1.0)  # Normalize to max expected linear vel
            actual_forward_pwm = max(self.fixed_forward_pwm * forward_scale, self.fixed_forward_pwm * 0.8)  # Min 80% of fixed
        ...
        pwm_for_drivetrain_left_arg  = pwm_l_orig # This goes to drivetrain's "left_motor" (NEW RIGHT)
        pwm_for_drivetrain_right_arg = pwm_r_orig # This goes to drivetrain's "right_motor" (NEW LEFT)
        ...
        self.drivetrain.set_speeds(pwm_for_drivetrain_left_arg, pwm_for_drivetrain_right_arg)
...
def main(args=None):
    ...
    try:
        ...
        if node: node.get_logger().info('Keyboard interrupt, shutting down motor controller.')
    except SystemExit:

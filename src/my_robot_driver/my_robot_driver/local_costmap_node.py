...
class LocalCostmapNode(Node):
    def __init__(self):
        ...
        self.final_costmap_data = np.full((self.height_cells, self.width_cells), self.COST_NEUTRAL_NP, dtype=np.int8)
        ...
        self.get_logger().info(f"LocalCostmapNode initialized. Costmap frame_id: '{self.global_frame}'.")
    ...
    def _compute_cost(self, distance_cells: int) -> np.int8:
        if distance_cells == 0: return self.sensor_obstacle_cost 
        if distance_cells > self.inflation_radius_cells: return self.COST_NEUTRAL_NP
        factor = (1.0 - float(distance_cells) / self.inflation_radius_cells)**2
        cost = (self.max_inflation_cost_val - self.COST_NEUTRAL_NP) * factor + self.COST_NEUTRAL_NP
        return np.int8(max(self.COST_NEUTRAL_NP, min(cost, self.max_inflation_cost_val)))

    def _inflate_obstacles(self, obstacle_marked_layer: np.ndarray) -> np.ndarray:
        if self.inflation_radius_cells <= 0: return obstacle_marked_layer 
        inflated_costmap = np.copy(obstacle_marked_layer)
        queue = collections.deque()
        cell_distances = np.full((self.height_cells, self.width_cells), float('inf'), dtype=float)
        for r in range(self.height_cells):
            for c in range(self.width_cells):
                if obstacle_marked_layer[r, c] == self.sensor_obstacle_cost:
                    queue.append((r, c, 0)); cell_distances[r,c] = 0
        while queue: 
            curr_r, curr_c, dist = queue.popleft()
            if dist >= self.inflation_radius_cells: continue
            for dr in [-1, 0, 1]:
                for dc in [-1, 0, 1]:
                    ...
                    if 0 <= nr < self.height_cells and 0 <= nc < self.width_cells:
                        ...
                        if new_dist_to_obstacle < cell_distances[nr, nc] :
                            ...
                            if inflated_costmap[nr, nc] < self.sensor_obstacle_cost:
                                inflated_costmap[nr, nc] = self._compute_cost(int(new_dist_to_obstacle))
    ...
    def update_and_publish_costmap(self):
        ...
        try:
            ...
            self.get_logger().warn(f"LocalCostmap TF lookup failed: {e}", throttle_duration_sec=1.0)
            return
        ...
        current_sensor_obstacle_layer = np.full((self.height_cells, self.width_cells), self.COST_NEUTRAL_NP, dtype=np.int8)
        ...
        for i, range_val in enumerate(self.latest_scan.ranges):
            ...
            try:
                ...
                p_transformed_to_global_frame = tf2_geometry_msgs.do_transform_point(p_lidar_frame_stamped, transform_lidar_to_global)
            except Exception as e:
                self.get_logger().warn(f"Failed to transform lidar point: {e}", throttle_duration_sec=1.0)
                continue
            
            cell_x = int((p_transformed_to_global_frame.point.x - costmap_origin_x_in_global_frame) / self.resolution)
            cell_y = int((p_transformed_to_global_frame.point.y - costmap_origin_y_in_global_frame) / self.resolution)
            
            if 0 <= cell_x < self.width_cells and 0 <= cell_y < self.height_cells:
                current_sensor_obstacle_layer[cell_y, cell_x] = self.sensor_obstacle_cost

        self.final_costmap_data = self._inflate_obstacles(current_sensor_obstacle_layer)
        
        grid_msg = OccupancyGrid()
        grid_msg.header.stamp = self.get_clock().now().to_msg() # Costmap published with current time
